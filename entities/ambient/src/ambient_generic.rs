...
impl Entity for AmbientGeneric {
    ...
    fn precache(&mut self) {
        ...
        if !spawn_flags.intersects(SpawnFlags::START_SILENT) && self.looping.get() {
            self.active.set(true);
        }
    ...
    fn spawn(&mut self) {
        let spawn_flags = self.spawn_flags();
        self.attenuation = if spawn_flags.intersects(SpawnFlags::EVERYWHERE) {
            Attenuation::NONE
        } else if spawn_flags.intersects(SpawnFlags::SMALL_RADIUS) {
            Attenuation::IDLE
        } else if spawn_flags.intersects(SpawnFlags::MEDIUM_RADIUS) {
            Attenuation::STATIC
        } else if spawn_flags.intersects(SpawnFlags::LARGE_RADIUS) {
            Attenuation::NORM
        } else {
            Attenuation::STATIC
        };
        ...
        self.looping
            .set(!spawn_flags.intersects(SpawnFlags::NOT_LOOPING));
        self.precache();
    }
    ...
    fn used(&self, use_type: UseType, _: Option<&dyn Entity>, _: &dyn Entity) {
        ...
        self.vars()
            .with_spawn_flags(|f| f | SpawnFlags::START_SILENT.bits());
...
/// Export `ambient_generic` entity to the engine.
#[doc(hidden)]
#[macro_export]
macro_rules! export_ambient_generic {
    () => {
        $crate::export_entity!(ambient_generic, $crate::ambient_generic::AmbientGeneric);
    };
}
#[doc(inline)]
pub use export_ambient_generic as export;
