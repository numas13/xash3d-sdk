...
impl<T: Move> BaseDoor<T> {
    ...
    fn go_end(&self) {
        ...
        if self.door_move.is_reversable() {
            if let Some(activator) = self.activator.get() {
                ...
                if !sf.intersects(SpawnFlags::ONEWAY) && v.move_dir().y != 0.0 {
    ...
    fn hit_end(&self) {
        ...
        if spawn_flags.intersects(SpawnFlags::NO_AUTO_RETURN) {
            if !spawn_flags.intersects(SpawnFlags::USE_ONLY) {
        ...
        if let Some(net_name) = v.net_name() {
            if spawn_flags.intersects(SpawnFlags::START_OPEN) {
    ...
    fn hit_start(&self) {
        ...
        self.enable_touch
            .set(!spawn_flags.intersects(SpawnFlags::USE_ONLY));
    ...
    fn block(&self, other_v: &EntityVars) {
        ...
        if !self.spawn_flags().intersects(SpawnFlags::SILENT) {
            if let Some(noise_moving) = v.noise_moving() {
                engine.build_sound().channel_static().stop(noise_moving, v);
            }
        }
...
impl<T: Move> Entity for BaseDoor<T> {
    ...
    fn object_caps(&self) -> ObjectCaps {
        ...
        if self.spawn_flags().intersects(SpawnFlags::USE_ONLY) {
